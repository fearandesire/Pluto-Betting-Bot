import { NBA_ACTIVEMATCHUPS, ODDS_NBA, _, container, flatcache } from '#config'
...
export async function updateOdds() {
	...
	for (let [key, value] of Object.entries(allNBAOdds)) {
		...
		await db.manyOrNone(
			`
    UPDATE "${NBA_ACTIVEMATCHUPS}" SET teamoneodds = $1, teamtwoodds = $2 WHERE teamone = $3 AND teamtwo = $4
    `,
