...
import { presznGames } from '../../lib/preseason/nfl_preseason_games.js'
...
export default async function collectOdds() {
    ...
    // Lazy code to get pre-season games up for now
    if (!_.isEmpty(presznGames)) {
        preseasonFlag = true
        gamesArr = presznGames
        console.log(`Collected locally`)
    } else {
        // Fetch data from the API
        gamesArr = await fetch(apiUrl, {
            method: 'GET',
            headers,
        }).then((res) => res.json())
    }
    ...
    await Promise.map(filteredPastGames, async (game) => {
        ...
        await scheduleChannels(homeTeam, awayTeam, {
            cronStartTime,
        })
    ...
    if (await !inCache(`matchups`)) {
        ...
        await Log.Green(
            `[collectOdds] New matchup collection stored into cache`,
        )
    }
    await scheduleEmbed()
