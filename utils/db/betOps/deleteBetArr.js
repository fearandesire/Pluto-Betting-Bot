...
export async function deleteBetFromArray(
	...
	if (!silent) {
		try {
			...
			Log.Green(
				`[deleteBetArr.js] Successfully deleted bet #${betid} from Local Storage "NBAbetslips" array for ${userid}`,
			)
			var embedcontents = {
	...
	if (silent == true) {
		try {
			...
			deleteBetArrLog.error(
				`Error: Unable to locate bet #${betid} for deletion.\n${err}`,
			)
			return
		}
	}
