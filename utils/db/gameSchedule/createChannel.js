...
export async function createChannel(awayTeam, homeTeam) {
	...
	try {
		...
		const imgFile = await fetchVsImg(`${homeTeam} vs ${awayTeam}`)
		if (!imgFile) {
			await gameChan.send({ embeds: [gameInfo] })
		} else {
			const imageAttachment = new MessageAttachment(imgFile, 'matchup.jpg')
			gameInfo.setImage(`attachment://matchup.jpg`)
			await gameChan.send({ embeds: [gameInfo], files: [imageAttachment] })
		}
		await dmMe(
		...
		await gameCreateMonitor.ping({
			state: `fail`,
			message: `Error Creating Game Channel:\n${error}`,
		})
	}
}
