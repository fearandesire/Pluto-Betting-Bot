...
export async function resolveCompCron() {
    ...
    const { earliestRange, latestRange } = calculateCronRanges(earliestTime, latestTime);
    ...
    const rangesEnglish = `[resolveCompCron.js] Earliest Game Time Today: ${cronstrue.toString(earliestTime.cronstart)} || Latest Game Time Today: ${cronstrue.toString(latestTime.cronstart)}`;
 ...
 */
  function calculateCronRanges(earliestTime, latestTime) {
    const { earliestHour, latestHour, oneGame, cMonth } = getGameHours(earliestTime, latestTime);
  
    const earliestRange = getCronRange(earliestHour, cMonth, earliestTime.dayOfMonth, earliestTime.dayOfWeek, '0-4');
  
    const latestRange = oneGame ?
      getCronRange(latestHour, cMonth, latestTime.dayOfMonth, latestTime.dayOfWeek) :
      getCronRange(latestHour, cMonth, latestTime.dayOfMonth, latestTime.dayOfWeek, '0-4');
 ...
 */
  ...
  function getCronRange(hour, month, dayOfMonth, dayOfWeek, range = `${hour}-${hour + 4}`) {
    return {
