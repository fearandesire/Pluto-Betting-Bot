...
const REMOTE_SPEC_URL = process.env.GORACLE_SPEC_URL || DEFAULT_REMOTE_SPEC_URL
...
async function writeSpec(content, source) {
	...
	console.log(`Goracle spec written from ${source}`)
...
main().catch((error) => {
	console.error('Unable to materialize Goracle spec', error)
	process.exit(1)
})
