...
export class ActivePropsService {
	...
	async refreshActiveProps(guildId: string): Promise<CachedProp[]> {
		try {
			...
			for (const dateGroup of dateGroups) {
				for (const game of dateGroup.games) {
					...
					for (const prop of game.props) {
						...
						const outcomeUuids = prop.outcome_uuids || [prop.outcome_uuid];
			...
			throw new Error(`Failed to refresh active outcomes for guild ${guildId}: ${error instanceof Error ? error.message : String(error)}`);
		}
	}
	...
	async getActiveProps(
		...
		// refreshActiveProps already returns the outcomes and updates cache
		const outcomes = await this.refreshActiveProps(guildId);
		return { props: outcomes, fromCache: false };
	}
}