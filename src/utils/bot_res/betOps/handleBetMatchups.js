...
export async function handleBetMatchups() {
	...
	await db.tx(`handleBetMatchups`, async (t) => {
		...
		const betPromises = await Object.entries(filteredForCompleted).map(
			async ([, MATCHUP]) => {
				const { id } = MATCHUP
				...
				const matchInfo = await MatchupManager.getViaId(id, t)

				if (!matchInfo) {
					return
				}

				const isBeingClosed = await closingQueue.inProgress(id, t)
				if (isBeingClosed) {
					await PlutoLogger.log({
						id: 3,
				...
				const betsExisting = await MatchupManager.outstandingBets(
					matchInfo.id,
					t,
				)
				if (!betsExisting) {
					...
					const aTeamShortName = getShortName(MATCHUP.away_team)
					const hTeamShortName = getShortName(MATCHUP.home_team)
					...
					await new ChannelManager().queueDeleteChannel(
						channelTitle,
						MATCHUP.id,
					)
