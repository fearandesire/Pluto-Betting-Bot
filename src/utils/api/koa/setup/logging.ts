...
export function createLoggingMiddleware() {
	return (str: string, args: any[]) => {
		...
		const userAgent = ctx?.state?.userAgent || ctx?.request?.headers?.['user-agent'] || 'unknown'
		const serviceName = ctx?.state?.serviceName || ctx?.request?.headers?.['x-service-name'] || 'unknown'
