...
import type { BetNotificationPush, NotifyBetUsers } from './notifications.interface.js';

export function validateNotifyBetUsers(payload: unknown): NotifyBetUsers | null {
	...
	const transformedPushes: BetNotificationPush[] | undefined = result.data.pushes?.map(push => ({
		userId: push.userid,
		betId: push.betid,
		result: {
